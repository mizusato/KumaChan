...
func GenerateApiDocs(idx checker.Index) ApiDocIndex {
	...
	for _, mod := range idx {
		...
		var outline_add_item = func(kind ApiItemKind, sym def.Symbol, sec ([] string)) {
		...
		var add_type = func(sym def.Symbol, g *checker.GenericType) {
		...
		var add_function = func(name string, group ([] checker.CheckedFunction)) {
			var sym = def.MakeSymbol(mod_name, name)
...
func typeDecl(sym def.Symbol, g *checker.GenericType, reg checker.TypeRegistry, mod string) Html {
	return block("type",
		block("header",
...
func funcDecl(sym def.Symbol, group ([] checker.CheckedFunction)) Html {
...
func funcOverload(sym def.Symbol, f checker.CheckedFunction) Html {
...
func funcAlias(mod string, alias_list ([] string)) Html {
	...
	for i, alias := range alias_list {
		var id = IdWithPrefix(def.MakeSymbol(mod, alias).String(), FuncDecl)
...
func link(sym def.Symbol, kind ApiItemKind, current_mod string) Html {
...
func sortSymbols(list ([]def.Symbol)) {
	sort.Sort(SymbolList(list))
}

type SymbolList ([]def.Symbol)
