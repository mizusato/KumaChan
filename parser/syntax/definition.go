...
var SyntaxDefinition = [...] string {
    /* Group: Root */
    "eval = resolve imports decls commands",
    "module = shebang export resolve imports decls commands",
      "shebang? = Pragma",
      "export? = @export { namelist! }! | @export namelist!",
        "namelist = name namelist_tail",
        "namelist_tail? = , name! namelist_tail",
        "name = Name",
      "resolve? = @resolve { extern_list }!",
        "extern_list = name", // TODO
    ...
    "operand = operand_body plain_calls with pipelines",
      "operand_body = unary operand_base operand_tail",
        "unary? = @not | _exc2 | - ",
        "operand_base = par | literal | create | cast | misc_expr | var",
          "par = lambda | wrapped",
          ...
          "cast = [ type ]! cast_flag (! expr! )!",
            "cast_flag? = ? | _exc1",
          "misc_expr = type_expr | await | text | when | match | iife",
            "type_expr = @type { type }",
            "await = @await",  // TODO
      ...
      "plain_calls? = plain_call plain_calls",
        "plain_call = Call ( arglist )! | { seq }!",
          "arglist? = exprlist",
          "seq? = seq_pair | seq_single",
            "seq_pair = pairlist",
