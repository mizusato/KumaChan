...
func (impl UntypedPipelineLambda) SemiExprVal() {}
type UntypedPipelineLambda struct {
	Pipeline  [] ast.VariousPipe
}
...
func (impl PipelineLambdaArgument) ExprVal() {}
type PipelineLambdaArgument struct {}
...
func CheckPipelineLambda(pl ast.PipelineLambda, ctx ExprContext) (SemiExpr, *ExprError) {
	var info = ctx.GetExprInfo(pl.Node)
	return SemiExpr {
		Value: UntypedPipelineLambda {
...
func AssignLambdaTo(expected Type, lambda UntypedLambda, info ExprInfo, ctx ExprContext) (Expr, *ExprError) {
	...
	case *AnonymousType:
		...
		case Func:
			...
			//
			var inner_ctx = ctx.WithPatternMatching(pattern, nil)
...
func AssignPipelineLambdaTo(expected Type, pl UntypedPipelineLambda, info ExprInfo, ctx ExprContext) (Expr, *ExprError) {
	var err = RequireExplicitType(expected, info)
	if err != nil { return Expr{}, err }
	switch E := expected.(type) {
	case *AnonymousType:
		switch func_repr := E.Repr.(type) {
		case Func:
			var input_t, err1 = GetCertainType (
				func_repr.Input, info.ErrorPoint, ctx,
			)
			if err1 != nil { return Expr{}, err1 }
			var output_expected = func_repr.Output
			var arg = LiftTyped(Expr {
				Type:  input_t,
				Value: PipelineLambdaArgument {},
				Info:  info,
			})
			var output_semi, err2 = CheckPipeline(arg, pl.Pipeline, ctx)
			if err2 != nil { return Expr{}, err2 }
			var output, err3 = AssignTo(output_expected, output_semi, ctx)
			if err3 != nil { return Expr{}, err3 }
			var output_certain, err4 = GetCertainType(output_expected, info.ErrorPoint, ctx)
			if err4 != nil { panic("something went wrong") }
			var _, ok = AssignType(output_certain, output.Type, ToInferred, ctx)
			if !(ok) {
				panic("type system internal error (likely a bug)")
			}
			return Expr {
